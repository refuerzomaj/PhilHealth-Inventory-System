...
$(document).on('submit', '#add-item-form', function (event) {
	...
	var form_data = new Array(
		$('input[id=itemname]'),
		$('input[id=om_amount]'),
		$('input[id=ismd_amount]'),
		$('input[id=itrmd_amount]'),
		$('input[id=ippsd_amount]'),
		$('input[id=purDate]'),
		$('#empID'),
		$('#catID')
	);
...
function fill_update_modal(iID) {
	$.ajax({
		...
		success: function (data) {
			...
			$('#om_amount-update').val(data.om_amount);
			$('#ismd_amount-update').val(data.ismd_amount);
			$('#itrmd_amount-update').val(data.itrmd_amount);
			$('#ippsd_amount-update').val(data.ippsd_amount);
...
$(document).on('submit', '#update-item-form', function (event) {
	...
	var form_data = new Array(
		...
		$('input[id=om_amount-update]'),
		$('input[id=ismd_amount-update]'),
		$('input[id=itrmd_amount-update]'),
		$('input[id=ippsd_amount-update]'),
...
//Add new request
$(document).on('submit', '#add-req-form', function (event) {
	event.preventDefault();
	/* Act on the event */
	var validate = '';
	var form_data = new Array(
...
//request acceptting
function request_done(req_id, item_id, off_desc, am) {
	request_id = req_id;
	item_ID = item_id;
	...
	// $('#modal-message-box').modal('show');
	// $('#modal-message-box').find('.modal-body').text("Request Accepted");
}
